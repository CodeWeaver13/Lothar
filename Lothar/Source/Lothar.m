          ...
          shouldCacheTarget:(BOOL)shouldCacheTarget
{
    ...
    if ([target respondsToSelector:action]) {
        return [self safeTarget:target performAction:action params:params];
    } else {
        // 这里是处理无响应请求的地方，如果无响应，则尝试调用对应target的notFound方法统一处理
        SEL notFound = NSSelectorFromString(@"notFound:");
        if ([target respondsToSelector:notFound]) {
            return [self safeTarget:target performAction:notFound params:params];
        } else {
...
- (id)safeTarget:(NSObject *)target performAction:(SEL)action params:(NSDictionary *)params {
